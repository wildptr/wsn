...
and parse_item env lexer : item =
  ...
  | LBRACE ->
      ...
      let outer_item = Nonterminal outer_name in
      Hashtbl.add env.nonterminal_table outer_name [[]; [Nonterminal inner_name; outer_item]];
      outer_item
...
let parse lexer : (string, nonterminal) Hashtbl.t =
  let env = {
